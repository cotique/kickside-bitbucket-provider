...
The executable example is `cotique/bitbucket` until initialized:

- `cotique.bitbucket:definition` is the authoritative root `ns.definition`.
- `cotique.bitbucket:log` is a typed automation destination port.
- `cotique.bitbucket:log_sink` binds the shared `kickside.data:writable` contract.
- `cotique.bitbucket.sink:write` validates and persists one acknowledged write.
- `cotique.bitbucket.persist:repo` owns SQL access.
- `cotique.bitbucket.migrations:01_create_log_entries` supports SQLite and Postgres.
- `cotique.bitbucket.api:get_status.endpoint` exposes authenticated module status.
- `cotique.bitbucket:bitbucket_view` publishes an announced, auto-registered Wippy
  web component served by the module's own embedded filesystem.
- `cotique.bitbucket.blocks:block.write_log` contributes the capability to the
